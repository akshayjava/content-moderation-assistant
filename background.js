...
class ModerationBackground {
    ...
    async generateReport(data, sender) {
        try {
            ...
            // Copy to clipboard
            await this.copyToClipboard(sender.tab.id, this.formatReport(report));
...
`;
    ...
    setupContextMenuHandler() {
        chrome.contextMenus.onClicked.addListener((info, tab) => {
            switch (info.menuItemId) {
                case 'flag-content':
                    this.performContextAction('flag', tab, info.selectionText);
                    break;
                case 'escalate-content':
                    this.performContextAction('escalate', tab, info.selectionText);
                    break;
                case 'block-user':
                    this.performContextAction('block', tab, info.selectionText);
                    break;
                case 'generate-report':
                    this.performContextAction('generate-report', tab, info.selectionText);
                    break;
    ...
    setupCommandsListener() {
        chrome.commands.onCommand.addListener(async (command, tab) => {
            ...
            if (action) {
                const selection = await this.getSelection(tab.id);
                this.performContextAction(action, tab, selection);
            }
        });
    }
    ...
    async performContextAction(action, tab, selectionText = '') {
        try {
            const data = {
                action: action,
            ...
            // Note: The content script needs to handle these specific actions
